...
const GPTMovieSuggestions = () => {
  ...
  if (!searchResults?.length && gptSearchText) {
    return (
      ...
      <h2  className="text-3xl font-bold py-3 text-white text-center">{LanguageConstants[language].NoResultsText}</h2>
      <p  className='text-white text-center text-lg p-3'>{LanguageConstants[language].NoResultsSuggestion}</p>
    </div>
    )
  }
  return   (
    <div className='bg-black w-8/12 m-auto'>
